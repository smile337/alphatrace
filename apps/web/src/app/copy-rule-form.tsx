...
export function CopyRuleForm(props: { walletAddress: string; plan: SubscriptionPlan }) {
  ...
  return (
    <form className="rule-grid" onSubmit={handleSubmit}>
      <p className={permission.allowed ? "permission-note" : "permission-note locked"}>{permission.message}</p>
      <label>
        <span>单笔上限</span>
      ...
      <button className={`rule-save-button ${status}`} disabled={status === "saving" || !permission.allowed} type="submit">
        {status === "saving" ? <LoaderCircle size={16} /> : status === "saved" ? <Check size={16} /> : <ShieldCheck size={16} />}
        <span>{status === "saving" ? "保存中" : status === "saved" ? "已保存" : status === "failed" ? "重试保存" : "保存规则"}</span>
      </button>
    </form>
  );
}
